...
export function renderApp(container, props) {
  const { data, loading, error, retry, empty, currency } = props;
    ...
    el1.className = "app";
    renderHeader(el1, { onCreate: handleCreate });
    if (error) {
    ...
    if (!loading) {
    renderUserTable(el1, { users: data, onUpdate: handleUpdate, onDelete: handleDelete });
    } else {
    appendText(el1, null);
    }
...
export function renderHeader(container, props) {
  const { data, loading, error, retry, empty, currency, updateField } = props;
    ...
    el5.setAttribute("value", form.username);
    el5.addEventListener("input", (e) => {
        dispatchEvent("updateField", [(e.target) && (e.target.value)], e);
      });
    ...
    el6.setAttribute("value", form.email);
    ...
    el7.setAttribute("value", form.password);
...
export function renderUserTable(container, props) {
  const { data, loading, error, retry, empty, currency, updateField } = props;
    ...
    const empty = ((props.empty) == (null)) ? null : props.empty;
    ...
    el16.setAttribute("value", editForm.username);
    ...
    el19.setAttribute("value", editForm.email);
    ...
    el22.setAttribute("value", editForm.password);
    ...
    el26.addEventListener("click", (e) => { if (typeof cancelEdit === 'function') cancelEdit(e); });
